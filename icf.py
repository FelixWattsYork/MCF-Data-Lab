...
def fit_plot(xdata,xdata_masked, ydata, ydata_masked, fitdata, xl="", yl="",title="",width = 0):

	position_ymax = np.argmax(fitdata)
	f, axarr = plt.subplots(2, sharex=True)
	axarr[0].set_title(title)
	axarr[1].plot(xdata, ydata, lw=2, label="Data")
	axarr[1].plot(xdata_masked, fitdata, lw=2, label="Fit")
	axarr[1].legend()
	axarr[1].set_xlabel(xl)
	axarr[1].set_ylabel(yl)	
	axarr[0].plot(xdata_masked, ydata_masked-fitdata, lw=2, label="Residual") 				
